...
public class JAVA0709LOrderSOOP01System {
	
	// Field
	Scanner scan;
	JAVA0709LOrderSOOP06LunchOrderMenuManager menuManager;
	
	String[] lunchMenuNames = {"햄버거(🍔)", "피자요(🍕) ", "라멘요(🍜)", "샐러드(🥗)"}; // < 05. 02. 데이터 정립
	int[] lunchMenuPrices = {100, 200, 300, 400}; // 05. 02. >
	JAVA0709LOrderSOOP03LunchMenu[] lunchMenuList;
	JAVA0709LOrderSOOP04LunchOrderItem[] orderItemList;
	JAVA0709LOrderSOOP05LunchPaymentItem paymentItem;
	int orderCount = 0;
	...
	// Constructor
	public JAVA0709LOrderSOOP01System() {
		scan = new Scanner(System.in);
//		menuManager = new JAVA0709LOrderSOOP06LunchOrderMenuManager(lunchMenuNames, lunchMenuPrices);
		menuManager = new JAVA0709LOrderSOOP06LunchOrderMenuManager(this);
		lunchMenuList = new JAVA0709LOrderSOOP03LunchMenu[4];
...
//		showMainMenu();
	...
	public JAVA0709LOrderSOOP01System(String title) {
		this.title = title;
		scan = new Scanner(System.in);
//		menuManager = new JAVA0709LOrderSOOP06LunchOrderMenuManager(lunchMenuNames, lunchMenuPrices);
		menuManager = new JAVA0709LOrderSOOP06LunchOrderMenuManager(this);
	...
	public int searchOrderItemIdx(int lunchMenu) {
		...
		for (int i = 0; i < orderCount; i++ ) {
			JAVA0709LOrderSOOP04LunchOrderItem orderItem = orderItemList[i];
...
//		}
		for(JAVA0709LOrderSOOP04LunchOrderItem orderItem : orderItemList) {
	...
	public void order(int lunchMenu) {
		...
		for(JAVA0709LOrderSOOP03LunchMenu menu : lunchMenuList) {
			if(menu.no == lunchMenu) {
				int idx = searchOrderItemIdx(lunchMenu);
				if(idx == -1) {
					orderItemList[orderCount] = new JAVA0709LOrderSOOP04LunchOrderItem();
		...
		if(orderCount == 0) {
            ...
            if (scan.hasNextInt()) {
                ...
                if (amount >= total) {
                    ...
                    paymentItem = new JAVA0709LOrderSOOP05LunchPaymentItem();
