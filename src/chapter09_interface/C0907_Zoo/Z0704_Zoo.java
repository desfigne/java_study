...
//		lion2.eat();
		...
		List<Z0703_Lion> list = new ArrayList<Z0703_Lion>(); // 라이언 단일만 리스트에 포함됨
		List<Z0702_Interface> list2 = new ArrayList<Z0702_Interface>(); // 라이언, 타이거 모두 리스트에 포함됨
		
		Z0703_Lion lion1 = new Z0703_Lion();
		Z0703_Tiger tiger1 = new Z0703_Tiger();
		list.add(lion1);
...
//		list2.get(0).name; // 인터페이스를 통해서 가져오기 때문에 이름이라는 정보는 없음, 에러 발생
		Z0703_Lion lion2 = (Z0703_Lion)list2.get(0); // 객체의 저장된 속성(값)은 강제(명시적) 형변환을 통해 진행
